...
const ManageIncidenceModal = ({
  ...
  const locationDisplay = incidence.location_type === 'habitacion' ? `Habitación ${incidence.room_number || ''}` : (LOCATION_LABELS[incidence.location_type] || incidence.location_type);

  return (
    <div className={UI_CLASSES.modalOverlay} onClick={onClose}>
      <div 
        className={UI_CLASSES.modalContainer} 
        ...
        <div className={UI_CLASSES.modalPadding}>
          <div className="mb-6 pr-8">
            <div className="flex items-center gap-2 mb-1">
              ...
              <h2 className={UI_CLASSES.modalTitle}>Gestionar Incidencia</h2>
            </div>
          ...
          <div className="space-y-5 max-h-[60vh] overflow-y-auto pr-2 custom-scrollbar">
            {incidence.img && (
              <section className="flex justify-center mb-2">
                <div className="rounded-[24px] overflow-hidden border border-slate-100 shadow-sm max-w-[200px] bg-slate-50">
                  <img src={incidence.img} alt="Evidencia" className="w-full h-auto object-contain max-h-[160px] cursor-zoom-in" onClick={() => window.open(incidence.img, '_blank')} />
                </div>
              </section>
            )}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className={UI_CLASSES.label}>Estado</label>
                <IncidenceSelect value={status} onChange={(val) => setStatus(val as IncidenceStatus)} options={STATUS_STYLES} placeholder="Estado" />
              </div>
              <div>
                <label className={UI_CLASSES.label}>Asignar Personal</label>
                <select 
                  value={staffId === "" && externalName !== "" ? "external_placeholder" : staffId}
                  onChange={(e) => {
                    const val = e.target.value;
                    if (val === "external_placeholder") { setStaffId(""); } 
                    else { setStaffId(val); setExternalName(""); }
                  }}
                ...
                >
                  ...
                  <option value="external_placeholder">+ Personal Externo</option>
                </select>
              </div>
            </div>
            ...
            {staffId === "" && (
               <div className="animate-in fade-in slide-in-from-top-1">
                  ...
                  <input type="text" value={externalName} onChange={(e) => setExternalName(e.target.value)} placeholder="Ej: Cerrajero..." className={UI_CLASSES.input} />
               </div>
            )}
            ...
            <div>
              ...
              <div className="relative">
                ...
                <div className="absolute right-3 bottom-3 text-slate-300"><Send size={16} /></div>
              </div>
            </div>
          </div>
...
const UI_CLASSES = {
  ...
  card: "bg-white rounded-[32px] p-6 shadow-sm border border-slate-100 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex flex-col justify-between h-full",
  ...
  modalContainer: "bg-white w-full max-w-lg max-h-[95vh] rounded-[35px] overflow-hidden shadow-2xl relative flex flex-col animate-in zoom-in duration-200",
  modalPadding: "p-6 sm:p-10 flex flex-col h-full",
  modalTitle: "text-xl font-black text-slate-900",
  modalIndicator: "w-2.5 h-2.5 rounded-full bg-[#82D14C]",
  label: "text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 block ml-1",
  input: "w-full bg-slate-50 border border-slate-100 rounded-2xl h-12 px-4 font-bold text-slate-700 outline-none focus:ring-2 focus:ring-emerald-500 text-sm",
  select: "w-full bg-slate-50 border border-slate-100 rounded-2xl h-12 px-4 font-bold text-slate-700 outline-none focus:ring-2 focus:ring-emerald-500 text-sm",
