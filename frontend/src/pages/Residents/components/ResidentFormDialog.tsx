...
function validate(form: FormState, isEdit: boolean): FieldErrors {
  ...
  if (!isEdit && form.bedroom_id === null) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[500px] max-h-[90vh] overflow-y-auto">
        ...
        <div className="grid gap-4 py-2">
          ...
          {/* Bedroom dropdown */}
          <div className="space-y-1.5">
            <Label htmlFor="res-bedroom">
              Habitación {!isEdit && <span className="text-red-600">*</span>}
            </Label>
            {bedroomsLoading ? (
              ...
              <NativeSelect
                ...
                aria-invalid={!!errors.bedroom_id}
                onChange={(e) =>
                  set("bedroom_id", e.target.value ? Number(e.target.value) : null)
                }
              >
                <option value="">{isEdit ? "Sin asignar" : "Selecciona una habitación"}</option>
                {bedrooms.map((b) => (
            ...
            {errors.bedroom_id && (
              <p className="text-xs text-red-600">{errors.bedroom_id}</p>
            )}
