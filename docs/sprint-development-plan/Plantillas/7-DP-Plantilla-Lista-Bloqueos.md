<h1>Plantilla — Lista de Bloqueos – NexUS</h1>

<p align="center">
  <img src="../../images/logo-app.png" alt="Logo NexUS" width="500">
</p>

<div align="center">

<p>
  <img src="https://img.shields.io/badge/Versión-1.0.0-blue?style=flat-square" alt="Versión">
  <img src="https://img.shields.io/badge/Estado-Plantilla-grey?style=flat-square" alt="Estado">
  <img src="https://img.shields.io/badge/Grupo-7--NexUS-green?style=flat-square" alt="Grupo">
  <img src="https://img.shields.io/badge/Asignatura-ISPP-red?style=flat-square" alt="Asignatura">
</p>

</div>

---

**Proyecto:** NexUS
**Grupo:** 7 - NexUS
**Asignatura:** Ingeniería del Software y Práctica Profesional (ISPP)
**Institución:** ETSII – Universidad de Sevilla
**Curso académico:** 2025/2026
**Sprint:** SX

<p align="center">
  <img src="../../images/logo-etsii.jpe" alt="Logo ETSII" width="400">
</p>

---

## Historial de Versiones

| Versión | Fecha | Cambio principal |
|---------|-------|------------------|
| 1.0.0 | DD/MM/AAAA | Creación del documento |

---

## Índice

1. [Descripción](#1-descripción)
2. [Tabla de Bloqueos](#2-tabla-de-bloqueos)
3. [Resumen por Impacto](#3-resumen-por-impacto)

---

## 1. Descripción

Registro centralizado de todos los impedimentos que han bloqueado o ralentizado el progreso del equipo durante el sprint. El Scrum Master es responsable de actualizar este documento y de trabajar en la resolución de los bloqueos con los responsables asignados.

**Niveles de impacto:**
- 🔴 **Alto** — Bloquea completamente una o más historias de usuario Must
- 🟡 **Medio** — Ralentiza el progreso pero existe workaround temporal
- 🟢 **Bajo** — Impacto menor, puede resolverse sin urgencia

**Estados:**
- 🚫 **Abierto** — No resuelto, activo
- 🔧 **En progreso** — Se está trabajando en ello
- ✅ **Resuelto** — Impedimento eliminado

---

## 2. Tabla de Bloqueos

| ID | Descripción | Estado | Responsable | Fecha Detección | Fecha Resolución | Impacto | Historia Afectada |
|----|-------------|--------|-------------|-----------------|------------------|---------|-------------------|
| BLQ-001 | [Descripción del bloqueo] | 🚫 Abierto | [Nombre] | DD/MM/AAAA | — | 🔴 Alto | S1-XX |
| BLQ-002 | [Descripción del bloqueo] | 🔧 En progreso | [Nombre] | DD/MM/AAAA | — | 🟡 Medio | S1-XX |
| BLQ-003 | [Descripción del bloqueo] | ✅ Resuelto | [Nombre] | DD/MM/AAAA | DD/MM/AAAA | 🟢 Bajo | S1-XX |

---

## 3. Resumen por Impacto

| Impacto | Total | Resueltos | Pendientes |
|---------|-------|-----------|------------|
| 🔴 Alto | 0 | 0 | 0 |
| 🟡 Medio | 0 | 0 | 0 |
| 🟢 Bajo | 0 | 0 | 0 |
| **Total** | **0** | **0** | **0** |
