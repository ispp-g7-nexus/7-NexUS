...
from apps.incidences.serializers import IncidenceSerializer, IncidenceUpdateSerializer, AdminIncidenceSerializer
...
class IncidenceSerializersTests(TestCase):
    def setUp(self):
        ...
        self.admin_user.is_staff = True
        self.admin_user.memberships.filter.return_value.values_list.return_value = ["admin"]
    ...
    def test_validate_modify_fields_as_owner(self):
        ...
        request.user = self.user
        serializer = IncidenceSerializer(instance=self.incidence, data={"title": "New Title"}, context={"request": request}, partial=True)
    ...
    def test_validate_non_admin_cannot_change_status(self):
        ...
        serializer = IncidenceSerializer(instance=inc_instance, data={"status": "resolved"}, context={"request": request}, partial=True)
        with self.assertRaises(ValidationError):
            serializer.is_valid(raise_exception=True)
    ...
    def test_validate_priority_owner_non_pending(self):
        ...
        serializer = IncidenceSerializer(instance=inc_instance, data={"priority": "low"}, context={"request": request}, partial=True)
        with self.assertRaises(ValidationError):
            serializer.is_valid(raise_exception=True)
...
class PermissionsTests(TestCase):
    ...
    def test_has_object_permission_admin(self):
        ...
        self.assertTrue(self.permission.has_object_permission(request, None, self.incidence))

    def test_has_object_permission_student_safe_method(self):
        request = self.factory.get("/")
        request.user = self.student
        self.assertTrue(self.permission.has_object_permission(request, None, self.incidence))

    def test_has_object_permission_student_put_pending(self):
        request = self.factory.put("/")
        request.user = self.student
        self.assertTrue(self.permission.has_object_permission(request, None, self.incidence))

    def test_has_object_permission_student_put_non_pending(self):
        request = self.factory.put("/")
        request.user = self.student
        self.incidence.status = "in_progress"
        self.assertFalse(self.permission.has_object_permission(request, None, self.incidence))

    def test_memberships_filter_raises_exception_safe_method_non_habitacion(self):
        bad_user = MagicMock()
        bad_user.id = 3
        bad_user.is_staff = False
        def raise_exc(*args, **kwargs):
            raise Exception("db error")
        bad_user.memberships.filter.side_effect = raise_exc
    ...
    def test_memberships_filter_raises_exception_safe_method_habitacion_non_owner(self):
        bad_user = MagicMock()
        bad_user.id = 3
        bad_user.is_staff = False
        bad_user.memberships.filter.side_effect = Exception("boom")
