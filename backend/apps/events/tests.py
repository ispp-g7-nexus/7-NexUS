...
class EventDetailViewTests(ViewTestsBase):
    ...
    def test_put_internal_errors(self):
        from apps.events.views import EventDetailView
        req1 = self.setup_request(self.rf.put('/', data=json.dumps({'start_time': (self.now + timedelta(hours=4)).isoformat(), 'end_time': (self.now + timedelta(hours=5)).isoformat(), 'space_id': 'abc'}), content_type='application/json'), user=self.host_user)
